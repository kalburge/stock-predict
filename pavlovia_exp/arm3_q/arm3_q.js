...
var conditions;
var condText = 'Frequency Level 2';
var resp;
var reward_pres;
var blockDisplay;
var blockN = 0;
...
async function experimentInit() {
  ...
  details2_left_box = new visual.Rect ({
    ...
    win: psychoJS.window,
    name: 'vars_text',
    text: "We will be manipulating the frequency of feedback for each of the 8 blocks of " + numSteps.toString() + " steps. The frequency levels range from low to high and will be shown on the slider below.",
    font: 'Open Sans',
    pos: [0, 0.41], height: 0.029,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: undefined,
    depth: 0.0
  });
  ...
  vars_left_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'vars_text',
    text: "High frequency indicates you will receive feedback often. Low frequency indicates you are likely to see 'No Feedback Given' often, regardless of the accuracy of your choice.",
    font: 'Open Sans',
    units: undefined, 
    pos: [-0.6, 0], height: 0.033,  wrapWidth: 0.25, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: undefined,
    depth: 0.0
  });
  ...
  vars_center_box = new visual.Rect ({
    win: psychoJS.window, name: 'side_box2', 
    ...
    win: psychoJS.window,
    name: 'vars_text',
    text: "You are in a slightly unstable environment. This means that the correct final state (UP or DOWN) will generally stay the same but may change at random with a 35% chance.",
    font: 'Open Sans',
  ...
  vars2_left_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'vars_text',
    text: "Your responses will tend to change between UP and DOWN periodically.",
    font: 'Open Sans',
    units: undefined, 
    pos: [-0.6, 0], height: 0.05,  wrapWidth: 0.25, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: undefined,
    depth: 0.0
  });
...
function directionsRoutineBegin(snapshot) {
  return async function () {
    ...
    directionsComponents.push(q_level);
    directionsComponents.push(q_dot);
...
function detailsRoutineBegin(snapshot) {
  return async function () {
    ...
    detailsComponents.push(q_level);
    detailsComponents.push(q_dot);
...
function feedbackExRoutineBegin(snapshot) {
  return async function () {
    ...
    feedbackComponents.push(q_level);
    feedbackComponents.push(q_dot);
...
function details2RoutineBegin(snapshot) {
  return async function () {
    ...
    details2Components.push(q_level);
    details2Components.push(q_dot);
...
function varsRoutineBegin(snapshot) {
  return async function () {
    ...
    varsComponents.push(vars_center_box);
    ...
    varsComponents.push(q_level);
    varsComponents.push(q_dot);
...
function varsRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && vars_center_box.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      vars_center_box.tStart = t;  // (not accounting for frame time here)
      vars_center_box.frameNStart = frameN;  // exact frame index
      vars_center_box.setAutoDraw(true);
    }
...
function vars2RoutineBegin(snapshot) {
  return async function () {
    ...
    vars2Components.push(q_level);
    vars2Components.push(q_dot);
...
function rewardRoutineBegin(snapshot) {
  return async function () {
    ...
    rewardComponents.push(q_level);
    rewardComponents.push(q_dot);
