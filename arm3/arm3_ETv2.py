...
def run_trial(stock, time, trial_index):
    ...
    plt.plot(range(time+1), stock[:time+1], linewidth=2.5)
    plt.xticks([])
...
#    event.clearEvents()  # clear cached PsychoPy events
    ...
    space_bar = False
    key = ''
    if time < WINDOW and trial < TRIALS:
...
#            break
    ...
    # Viewer User Manual, "Protocol for EyeLink Data to Viewer Integration"
    el_tracker.sendMessage('!V TRIAL_VAR current_time %s' % time)
    el_tracker.sendMessage('!V TRIAL_VAR current_price %s' % stock[time])
...
for trial in range(TRIALS):
    if run == 'space' and t < WINDOW + 1: 
        run = run_trial(stock, t, trial)
        responses.append(run)
        t += 1
    else:
        stock = stock = generate_stock(DRIFT, SIG, WINDOW)
        t = 1
        run = run_trial(stock, t, trial)
        responses.append(run)
        t += 1
