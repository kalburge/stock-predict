...
def run_trial(start_date, current_date, trial_index):
    ...
    msg = visual.TextStim(win, text=task_txt, height=0.085, pos=(0,-0.7))
...
#            break
    ...
    # Viewer User Manual, "Protocol for EyeLink Data to Viewer Integration"
    el_tracker.sendMessage('!V TRIAL_VAR start_date %s' % start_date)
    el_tracker.sendMessage('!V TRIAL_VAR current_date %s' % current_date)
...
task_msg = 'In the task, you may press the SPACE to end a trial\n' + \
...
# loading Nasdaq historical data and relevant derivative variables
...
for trial in range(TRIALS):
    ...
    run = run_trial(start_date,current_date,trial)
    current_date += 1
    responses.append([start_date, current_date, run])
    subtrial = 0
    while run == 'space' and current_date <= start_date + MAX_SAMP:
        subtrial += 0.1
        run = run_trial(start_date,current_date, trial+subtrial)
        current_date += 1
        responses.append([start_date, current_date, run])
