...
class Experiment(ABC):
    def __init__(self, N, mu, sigma, epsilon, q, R, c):
        ...
        self.R = R # reward tuple (reward, punishment, nothing)
...
#            feedback = np.random.binomial(1, self.q*(s_i != response[1])+(1-self.q)*(s_i == response[1]))
            draw = np.random.binomial(1, self.q)
            if draw:
                feedback = 1 - (s_i == response[1])
            else:
                feedback = 2
...
class RewardMax(Experiment):
    ...
    def bellmans(self):
        dp = 0.001
        R_c = self.R[0]; R_i = self.R[1]; R_0 = self.R[2]
        ...
        # Decide s_+, feedback +:
        p_0_pp = self.likelihood_prior_update(p, 1, 0)
        # Decide s_-, feedback +:
        p_0_mp = self.likelihood_prior_update(p, -1, 0)
        # Decide s_+, feedback -:
        p_0_pm = self.likelihood_prior_update(p, 1, 1)
        # Decide s_-, feedback -:
        p_0_mm = self.likelihood_prior_update(p, -1, 1)
        # Decide s_+, feedback 0:
        p_0_p0 = self.likelihood_prior_update(p, 1, 2)
        # Decode s_-, feedback 0:
        p_0_m0 = self.likelihood_prior_update(p, -1, 2)
        # Correct for q = (0, 1) edge cases:
        if self.q == 0: # WHAT IS THIS
        ...
        ind_0_p0 = sorter[np.searchsorted(p, np.around(p_0_p0/dp)*dp, sorter=sorter)]
        ind_0_m0 = sorter[np.searchsorted(p, np.around(p_0_m0/dp)*dp, sorter=sorter)]
        # Construct likelihood transfer matrix:
        F = self.likelihood_transfer(p, p)
        # Perform first step of backward induction:
        
        V_p = self.q*(p*R_c+(1-p)*R_i) + (1-self.q)*R_0 # Value of choosing s_+
        V_m = self.q*((1-p)*R_c+p*R_i) + (1-self.q)*R_0 # Value of choosing s_-
        ...
        for i in range(N_mod-1, 0, -1):
            ...
            if self.c[0] > N_mod-i:
                ...
                V_p = self.q*(p*(R_c + V[ind_0_pp, N_mod-i-self.c[0]])+(1-p)*(R_i+V[ind_0_pm, N_mod-i-self.c[0]])) + (1-self.q)*(R_0 + V[ind_0_p0, N_mod-i-self.c[0]]) # Value of choosing s_+
                V_m = self.q*((1-p)*(R_c+V[ind_0_mp, N_mod-i-self.c[0]])+p*(R_i+V[ind_0_mm, N_mod-i-self.c[0]])) + (1-self.q)*(R_0+V[ind_0_m0, N_mod-i-self.c[0]]) # Value of choosing s_-
    ...
    def likelihood_prior_update(self, p_d, d, r):
        ...
        if ((r == 0) & (d == -1)) | ((r == 1) & (d == 1)):
            p_0 = self.epsilon # Pr(s=s_+)/(Pr(s=s_pm))             (Pr(s=s_pm)) = 1
        return p_0
